...
function Initialize-KeyAuthSession($cfg) {
    ...
    $name = [uri]::EscapeDataString([string]$cfg.keyauth_name)
    $owner = [string]$cfg.keyauth_ownerid
    $ver = [uri]::EscapeDataString([string]$cfg.keyauth_version)
    $url = "${base}?type=init&name=$name&ownerid=$owner&ver=$ver"
...
function Get-KeyAuthVariable($session, $varId) {
    ...
    $vid = [uri]::EscapeDataString([string]$varId)
    $url = "$($session.BaseUrl)?type=var&varid=$vid&sessionid=$($session.SessionId)&name=$($session.Name)&ownerid=$($session.OwnerId)"
    $resp = Invoke-RestMethod -Uri $url -Method Get -TimeoutSec 20
    if ($resp.success) {
        return [string]$resp.message
    }
    Write-Log "KeyAuth var '$varId' failed: $($resp.message)"
    return $null
}

function Get-RemoteReleaseInfo($cfg) {
    $versionVar = if ($cfg.keyauth_var_version) { [string]$cfg.keyauth_var_version } else { 'streamer_version' }
    $urlVar = if ($cfg.keyauth_var_url) { [string]$cfg.keyauth_var_url } else { 'streamer_url' }

    try {
        $session = Initialize-KeyAuthSession $cfg
        $remoteVer = Get-KeyAuthVariable $session $versionVar
        $remoteUrl = Get-KeyAuthVariable $session $urlVar
