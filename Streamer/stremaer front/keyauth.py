...
class api:
    ...
    def init(self):
        ...
        if json["message"] == "invalidver":
            if json["download"] != "":
                ...
                update_ps1 = os.path.join(script_dir, "UPDATE.ps1")
                if os.path.isfile(update_ps1):
                    import subprocess
                    subprocess.run(
                        [
                            "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass",
                            "-File", update_ps1,
                            "-KeyAuthDownloadUrl", download_link,
                        ],
                        cwd=script_dir,
                    )
                else:
    ...
    def login(self, user, password, hwid=None):
        ...
        if json["success"]:
            ...
            # IMPORTANT: don't kill whole app process here.
            # Propagate the error so the caller (Flask route) can show message instead of crashing.
            msg = json.get("message", "Login failed")
            print(msg)
    ...
    def fetch_var(self, name):
        """Read app variable without killing the process on failure."""
        self.checkinit()
    ...
    def __do_request(self, post_data):
        try:
            ...
            debug_dir = f"C:\\ProgramData\\KeyAuth\\Debug\\{exe_name}"
            if not os.path.exists(debug_dir):
                os.makedirs(debug_dir)

            with open(os.path.join(debug_dir, "log.txt"), "a") as log_file:
                if len(response.text) <= 200:
                    tampered = not (signature and hmac.compare_digest(client_computed, signature))
            ...
            if not signature:
                raise RuntimeError("KeyAuth response missing signature header.")

            if not hmac.compare_digest(client_computed, signature):
                raise RuntimeError(
                    "Signature checksum failed. Request was tampered with or session expired."
                )

            return response.text
        except requests.exceptions.Timeout as e:
            raise RuntimeError("Request timed out. KeyAuth server slow or down.") from e
        except requests.exceptions.RequestException as e:
            raise RuntimeError(f"Network error talking to KeyAuth: {e}") from e
